...
public final class AnboxPlatformServiceProxy {
    ...
    private void addWindowStateToParcel(WindowState w, Parcel data) {
        ...
        data.writeInt(w.getWindowFrames().mFrame.left);
        data.writeInt(w.getWindowFrames().mFrame.top);
        data.writeInt(w.getWindowFrames().mFrame.right);
        data.writeInt(w.getWindowFrames().mFrame.bottom);
        Task task = w.getTask();
        if (task != null) {
         ...
         */
        data.writeInt(mWm.getDefaultDisplayRotation());
    }
    ...
    public void updateWindowState() {
        ...
        final int numDisplays = mWm.mRoot.getChildCount();
        data.writeInt(numDisplays);
        for (int n = 0; n < numDisplays; ++n) {
            final DisplayContent display = mWm.mRoot.getChildAt(n);
            data.writeInt(display.getDisplayId());

            final WindowList windows = mWm.mRoot.mChildren;
            int windowSize = 0;
            for (int m = 0; m < windows.size(); ++m) {
                final WindowState w = ((DisplayContent)windows.get(m)).mCurrentFocus;
                if(w == null)
                    continue;
                windowSize++;
            }

            data.writeInt(windowSize);
            for (int m = 0; m < windows.size(); ++m) {
                final WindowState w = ((DisplayContent)windows.get(m)).mCurrentFocus;
                if(w == null)
                    continue;
        ...
        for (int n = 0; n < mRemovedWindows.size(); n++) {
            final WindowState w = (WindowState)mRemovedWindows.get(n);
            addWindowStateToParcel(w, data);
        }
